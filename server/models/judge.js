...
export default class Judge {
  static async create({ name, region, nnk, knk, jnk, kgj, kink, konk, ink }) {
    const id = nanoid(4);
    const judge = {
      id,
      name,
    ...
    return pick(data, ['id', 'name', 'region', 'nnk', 'knk', 'jnk', 'kgj', 'kink', 'konk', 'ink', '_etag']);
  }
  ...
  static async getAll() {
    const querySpec = { query: 'SELECT c.id, c.name, c.region, c.nnk, c.knk, c.jnk, c.kgj, c.kink, c.konk, c.ink, c._etag from c' };
