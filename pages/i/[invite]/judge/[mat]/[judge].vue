        ...
        <tbody class="bg-base-100">
          <tr v-for="(score, index) in scores.points" :class="techniqueColour(score)">
            ...
            <td v-for="(deduction, dIndex) in score.deductions || Array(6).fill(0)" class="score"
              @click.prevent="toggleScore(score, dIndex)" v-show="!disableScore(dIndex)">
              <div class="h-6 px-1">
...
function disableScore(index) {
  return (index === 3 && match.value.disableMajor) || (index === 4 && match.value.disableForgotten);
}
...
onMounted(async () => {
  ...
  event.connect((data) => {
    ...
    if (data.index !== matchIndex || data.groupIndex !== groupIndex) {
      ...
      if (!scores.value.points || scores.value.points.length === 0) {
